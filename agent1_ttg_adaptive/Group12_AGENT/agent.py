...
CHECKPOINT_PATH = os.path.join(
    ...
    "ray_results/checkpoint-3600",
)
POLICY_NAME = "default"


class LSTMAdaptiveTTGAgent(AgentInterface):
    """PPO + LSTM agent - optimized for low-memory inference."""
    ...
    def __init__(self, env: gym.Env):
        ...
        ray.init(ignore_reinit_error=True, _memory=500_000_000, object_store_memory=100_000_000)
        ...
        nvec_shape = self._action_space.nvec.shape if hasattr(self._action_space, 'nvec') else (3,)
        self._zero_action = np.zeros(nvec_shape, dtype=np.int64)
